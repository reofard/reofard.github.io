...
그 외에도 여러가지 방법을 통해 $s,t$를 구하여 손실함수를 정의할 수 있다.
...
MiDaS는 다양한 데이터셋를 통해 모델을 학습하여 일반화 성능을 올린다. 해당 논문에서는 다양한 데이터셋을 학습하기 위한 두가지 전략을 소개하는데 하나씩 확인해보자.

### **균등 혼합 학습 전략**

해당 학습 전략은 학습에 사용되는 각각의 미니배치를 여러 데이터셋을 동일한 비율로 섞어서 만드는 방법이다. 

### **Pareto-optimal 혼합 학습 전략**

해당 학습 전략은 [Multi-Task Learning as Multi-Objective Optimization](https://arxiv.org/abs/1810.04650)이라는 논문에 나오는 방법으로 학습을 시키는 방법이다. 한 데이터셋의 성능을 향상시키는 동시에 다른 데이터셋의 성능이 떨어지지 않도록 균형을 맞추는 전략이다.
...
이와 별개로 이 논문이 대단하다고 생각한 점은 다양한 형식의 데이터셋을 간단한 아이디어인 최소자승법과 geometry transformation만을 이용해 Zero Shot Learning을 구현했다는 점이라고 생각했다. 프로그래밍에서도 케이스마다 분기로 처리하는것보다 모든 케이스를 아우를 수 있는 핵심적인 알고리즘 하나로 개발하는게 더 좋듯이, 연구도 간단한데, 아무나 생각하지 못하는 핵심적인 아이디어 하나로 발전해 나가는것 이란 것을 배울 수 있었다.